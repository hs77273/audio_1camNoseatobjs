        ...
        if isinstance(object_names, list):
            ...
            for obj in object_names:
                if obj in OBJECT.offensive_objects:
                    colors.append("red")
                    text = f"offensive object {obj} detected"
                    if not audio_played:
                        audio_thread = threading.Thread(target=self.play, args=(text,))
                        audio_thread.start()
                        audio_played = True
                else:
                    colors.append("white")
            ...
            if not any(obj in OBJECT.offensive_objects for obj in object_names) and audio_played:
                audio_played = False
            ...
            if object_names in OBJECT.offensive_objects:
                ...
                if not audio_played:
                    audio_thread = threading.Thread(target=self.play, args=(f"offensive object {object_names} detected",))
                    audio_thread.start()
                    audio_played = True
            else:
                self.object_msg.setText(str(object_names) if str(object_names) != "" else "No Objects Detected")
                if not audio_played:
                    audio_thread = threading.Thread(target=self.play, args=("No Objects Detected",))
                    audio_thread.start()
                    audio_played = True

    def play(self, text):
        audio_stream = text_to_audio(text) 
